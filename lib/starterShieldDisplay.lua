...
--
-- USAGE:
-- Display = require "starterShildDisplay"
-- d = Display:new()
...
local INDEX_NEGATIVE_SIGN = 16
local INDEX_BLANK            = 17

-- tubeTab is 0-9, A, b, C, d, E, F, '-', ' '
...
function Display:new(pinclk, pindata)
     setmetatable(self, {})
    self.Clkpin = pinclk or PINCLK
    self.Datapin = pindata or PINDTA
    storm.io.set_mode(storm.io.OUTPUT, self.Clkpin)
    storm.io.set_mode(storm.io.OUTPUT, self.Datapin)
    self:set()
    return self
end
...
function Display:time(hour, min)
    ...
    self:clear()
    self:pointOn()
    self:display(4, hour / 10 == 0 and 0x7f or hour / 10)
...
function Display:writeByte(wr_data)
    ...
    for i=1,8 do      -- sent 8bit data
        storm.io.set(storm.io.LOW, cpin)
        if bit.band(wr_data, 0x01) == 1 then
            storm.io.set(storm.io.HIGH, dpin)  -- LSB first
        else
            storm.io.set(storm.io.LOW, dpin)
        end
        wr_data = bit.rshift(wr_data, 1)
        storm.io.set(storm.io.HIGH, cpin)
    end

    -- wait for the ACK
    storm.io.set(storm.io.LOW, cpin)
    storm.io.set(storm.io.HIGH, dpin)
    storm.io.set(storm.io.HIGH, cpin)
    storm.io.set_mode(storm.io.INPUT, dpin)

    while storm.io.get(dpin) ~= 0 do
        cord.await(storm.os.invokeLater, storm.os.MILLISECOND)
        storm.io.set_mode(storm.io.OUTPUT, dpin)
        storm.io.set(storm.io.LOW, dpin)
        storm.io.set_mode(storm.io.INPUT, dpin)
    end
    storm.io.set_mode(storm.io.OUTPUT, dpin)
...
function Display:start()
    storm.io.set(storm.io.HIGH, self.Clkpin)  --send start signal to TM1637
    storm.io.set(storm.io.HIGH, self.Datapin)
    storm.io.set(storm.io.LOW, self.Datapin)
    storm.io.set(storm.io.LOW, self.Clkpin)
end
...
function Display:stop()
    storm.io.set(storm.io.LOW, self.Clkpin)
    storm.io.set(storm.io.LOW, self.Datapin)
    storm.io.set(storm.io.HIGH, self.Clkpin)
    storm.io.set(storm.io.HIGH, self.Datapin)
end
